...
<!-- kodingan ini masih untuk yang login saja yang bisa like dan komen ya -->
...
<div class="container">
    <div class="row">
        <div class="col-6 mt-2">
            ...
            <div class="card-body">  
            <h3 class="card-title"><?= $data['judulfoto'] ?><a href="<?php if(isset($_SESSION['userid'])){echo '?url=like&&id='.$data['fotoid'].'';}else{echo 'login.php';} ?>" class="btn btn-sm 
            <?php if($cek==0){ echo "text-secondary"; }else{ echo "text-danger"; } ?>"><i class="fa-solid fa-fw fa-heart"></i> <?= $likes ?></a></h3
            
            <small class="text-muted mb-3">by: <?= $data['username'] ?>, <?= $data['tanggalunggah']?></small>
            <p><strong>Deskripsi:</strong> <?= $data['deskripsifoto']?></p>
            <!-- <p>lihat semua komentar: <? $data['fotoid']?></p> -->
            <?php
            // ambil data komentar
            $submit=@$_POST['submit'];
            if ($submit=='Kirim') {
